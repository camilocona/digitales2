...
MOV R7, #0 //se inicia en 0 para evitar numeros arbitrarios
...
_CONTADORES2_:
	...
	ADD R9, R9, #1 // Se incrementa para recorrer el while interno
...
_MULTI_: //MULTIPLOS DE 4
	MOV R9, #0x03
	CMP R3,R2 //R2 es i del while , inicia (0<i) 
	BGE Final  //Se va a Final cuando se cumpla la condicion de R3>=R2, sino continua
	
	LDR R4, [R1,R7] 
	AND R0, R4, R9 //Se hace la AND con los dos últimos bits en 1 (3) 
	
	CMP R0, #0 //Si la AND es cero, es múltiplo de 4, sino no es
